...
| **Tests** | 262 tests, run in CI on Python 3.12 to 3.14 without Docker or an API key. |
...
- **Demo target.** The lab is a small Docker stack, not a production estate. A target can
  ...
  Identity providers are detected and read through adapters (Keycloak and Okta so far);
